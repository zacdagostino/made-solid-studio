...
- Mark the surface with `data-sf-navigation-motion` and mark its real logo, each primary link, and each secondary action or close control with `data-sf-navigation-item`. The locked runtime begins revealing the decoded logo and essential close control with no deliberate delay as soon as the surface enters, starts the first route within 60ms, sequences the remaining items in reading order, and reverses them as it exits. Do not add competing delays or reveal every route at once.
...
Treat the open state as a short composition: surface, approved logo, and essential close control begin together; primary routes follow in a tight sequential reading order, then non-essential secondary actions. Use the runtime choreography rather than competing CSS transitions on those marked elements. Pacing should feel calm and responsive—never linear, bouncy, or delayed so long that navigation feels unavailable.
...
- immediate perceived response: the decoded logo has no deliberate transition delay, the first route begins within 60ms, and the remaining item delay is bounded so the drawer never presents an empty surface while navigation waits to start.
