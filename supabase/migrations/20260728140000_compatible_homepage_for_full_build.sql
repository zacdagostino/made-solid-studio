-- A complete build may reuse a reviewed homepage direction after approved logos or
-- recovered semantic content create a replacement manifest, provided both manifests
-- reference the same immutable capture and Research Packet. The full build remains
-- pinned to the current manifest and published agent package, and quality review
-- verifies the current assets and recovered-content coverage.
...
begin
  ...
  if source_run.id is null and requested_mode in ('page_test', 'full_site') then
    ...
    where candidate.business_id = target_business_id
      and (
        (
          requested_mode = 'page_test'
          and candidate.build_manifest_id = target_manifest.id
        )
        or (
          requested_mode = 'full_site'
          and exists (
            select 1
            from public.build_manifests as candidate_manifest
            where candidate_manifest.id = candidate.build_manifest_id
              and candidate_manifest.organization_id = target_organization_id
              and candidate_manifest.business_id = target_business_id
              and candidate_manifest.crawl_run_id = target_manifest.crawl_run_id
              and candidate_manifest.research_packet_id = target_manifest.research_packet_id
          )
        )
      )
      and candidate.build_mode in ('homepage_test', 'page_test')
