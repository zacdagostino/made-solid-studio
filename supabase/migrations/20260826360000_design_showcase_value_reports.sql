-- A new immutable report contract turns exact before-and-after evidence into a design showcase.
-- GPT-5.6 Sol identifies visible original-site problems and design decisions; deterministic gates
-- continue to own lineage, matched screens, page readiness and exact edited-release proof.
...
begin
  ...
  if not exists (
    ...
    raise exception 'No technology-verified before-and-after comparison candidates are available. Run release verification again.';
  end if;
  ...
  where business_id = target_business_id and audit_id = target_audit.id
    ...
    and generator_contract_version = 'client-value-report-agent-v2'
  for update;
  ...
  if existing_job.id is not null then
    return query update public.report_generation_jobs set
      ...
      progress_detail = 'Waiting for the protected design report worker.',
  ...
  return query insert into public.report_generation_jobs (
    ...
    'client-value-report-agent-v2', 'gpt-5.6-sol', 'max',
    'Waiting for the protected design report worker.'
  ) returning *;
end;
$$;
...
begin
  ...
  if target_report.review_state <> 'approved' or target_report.schema_version <> 10
     ...
     or target_report.data->>'generatorRevision' <> 'gpt-5.6-sol-design-showcase-v2' then
  ...
  if jsonb_array_length(coalesce(target_report.data->'majorFindings', '[]'::jsonb)) = 0
    ...
    raise exception 'The client report must contain verified design findings and finished before-and-after themes.';
  end if;
