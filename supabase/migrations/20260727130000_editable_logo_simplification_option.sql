alter table public.asset_analysis_jobs
  add column if not exists editable_logo_simplification_enabled boolean not null default false;

drop function if exists public.request_editable_logo_retry(uuid);
...
begin
  ...
  insert into public.asset_analysis_jobs (
    ...
    editable_logo_simplification_enabled
  ) values (
    target_organization_id, target_business_id, target_crawl_run_id, 'queued',
    target_asset_id, gen_random_uuid(), simplify_geometry
...
grant execute on function public.request_editable_logo_retry(uuid, boolean) to authenticated;
