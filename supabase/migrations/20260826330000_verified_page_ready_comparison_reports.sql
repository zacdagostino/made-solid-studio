-- A client comparison is valid only when the exact edited page is fully rendered at the same
-- viewport as the original evidence. Loading covers, overflowing redesigns, and cropped media are
-- excluded before a report version can be frozen.

alter function public.create_audit_report_version(uuid, uuid)
  rename to create_audit_report_version_v7_comparison;
revoke all on function public.create_audit_report_version_v7_comparison(uuid, uuid)
...
begin
  ...
  source_id := public.create_audit_report_version_v7_comparison(
    target_business_id,
    target_audit_id
  );
  ...
  where versions.business_id = target_business_id
    ...
    and versions.schema_version = 8
    and versions.data->>'generatorRevision' = 'verified-ready-design-comparison-v2'
  ...
  select coalesce(jsonb_agg(
    theme || jsonb_build_object(
      'afterEvidence', jsonb_build_object(
        ...
        'caption', 'The finished verified redesign at the same page and viewport.',
        'verification', jsonb_build_object(
          'status', 'passed',
          'captureContract', after_artifact.metadata->>'captureContract',
          'pageReady', true,
          'loaderVisible', false,
          'sameViewport', true,
          'originalHorizontalOverflowPx', case
            when coalesce(old_artifact.metadata->>'horizontalOverflowPx', '') ~ '^[0-9]+$'
              then (old_artifact.metadata->>'horizontalOverflowPx')::integer
            else 0
          end,
          'redesignedHorizontalOverflowPx',
            (after_artifact.metadata->>'horizontalOverflowPx')::integer
        )
      ),
      'comparison', coalesce(theme->'comparison', '{}'::jsonb) || jsonb_build_object(
        'whatChanged', case
          when coalesce(old_artifact.metadata->>'horizontalOverflowPx', '') ~ '^[0-9]+$'
            and (old_artifact.metadata->>'horizontalOverflowPx')::integer > 1
            then 'The redesign replaces the overflowing desktop-width layout with a page that fits the same phone viewport.'
          else coalesce(theme#>>'{comparison,whatChanged}',
            'The redesign creates a clearer hierarchy at the same verified viewport.')
        end,
        'whyBetter', case
          when coalesce(old_artifact.metadata->>'horizontalOverflowPx', '') ~ '^[0-9]+$'
            and (old_artifact.metadata->>'horizontalOverflowPx')::integer > 1
            then 'Phone visitors can now read and use the page without content being cut off-screen.'
          else coalesce(theme#>>'{comparison,whyBetter}',
            'Customers receive a clearer, more dependable experience at this screen size.')
        end,
        'verificationSummary', format(
          'Verified %s comparison at %s × %s after the loading cover completed.',
          coalesce(after_artifact.metadata#>>'{viewport,label}', 'responsive'),
          after_artifact.metadata#>>'{viewport,width}',
          after_artifact.metadata#>>'{viewport,height}'
        )
      )
    ) order by ordinality
  ), '[]'::jsonb)
  ...
  join lateral (
    ...
    where artifacts.organization_id = source_report.organization_id
      ...
      and artifacts.metadata->>'captureContract' = 'verified-comparison-page-ready-v1'
      and artifacts.metadata->>'captureStatus' = 'passed'
      and artifacts.metadata->>'pageReady' = 'true'
      and artifacts.metadata->>'loaderVisible' = 'false'
      and artifacts.metadata->>'routeTransitioning' = 'false'
      and coalesce(artifacts.metadata->>'horizontalOverflowPx', '') ~ '^[0-9]+$'
      and (artifacts.metadata->>'horizontalOverflowPx')::integer <= 1
      ...
      and artifacts.metadata#>>'{viewport,width}' = theme#>>'{evidence,viewport,width}'
      and artifacts.metadata#>>'{viewport,height}' = theme#>>'{evidence,viewport,height}'
    order by artifacts.created_at desc, artifacts.id
    limit 1
  ) after_artifact on true
  join public.artifacts old_artifact
    on old_artifact.id = (theme#>>'{evidence,artifactId}')::uuid
   and old_artifact.business_id = target_business_id
   and old_artifact.crawl_run_id = source_report.crawl_run_id;

  if jsonb_array_length(comparison_themes) = 0 then
    raise exception 'No finished, viewport-matched redesign screenshots are available. Run release verification again.';
  end if;
  ...
  revised_data := source_report.data || jsonb_build_object(
    ...
    'comparisonMethod', jsonb_build_object(
      ...
      'requiresFinishedPage', true,
      'requiresNoRedesignOverflow', true,
      'requiresExactVerifiedCommit', true
    ),
    'analysisProvenance', jsonb_build_object(
      'visualFindingModels', to_jsonb(array(
        select distinct observations.measurement->>'model'
        from public.audit_observations observations
        where observations.audit_id = target_audit_id
          and nullif(observations.measurement->>'model', '') is not null
        order by observations.measurement->>'model'
      )),
      'reportAssembly', 'deterministic evidence selection and client-safe copy rules',
      'comparisonVerification', 'Playwright exact-commit browser checks'
    )
  );
  ...
  insert into public.decision_report_versions (
    ...
    source_report.crawl_run_id, next_version, 8, 'approved',
    format('%s finished, viewport-matched design comparisons generated from the verified website.',
...
begin
  ...
  if target_report.review_state <> 'approved' or target_report.schema_version <> 8
     or target_report.data->>'reportKind' <> 'verified_redesign_value'
     or target_report.data->>'generatorRevision' <> 'verified-ready-design-comparison-v2' then
