-- A revision is intentionally pinned to a private, completed builder run. This
-- keeps a new package test from implicitly using whichever page happened to be
-- built most recently, and prevents public pages from becoming revision input.
drop function if exists public.request_website_build(uuid, text, text, text, uuid);
...
begin
  ...
  if source_run.id is null and requested_mode = 'homepage_test' then
    select * into source_run from public.builder_runs as candidate
    ...
    order by created_at desc limit 1;
  elsif source_run.id is null then
    select * into source_run from public.builder_runs as candidate
    where candidate.business_id = target_business_id and candidate.build_manifest_id = target_manifest.id
...
grant execute on function public.request_website_build(uuid, text, text, text, uuid, uuid) to authenticated;
